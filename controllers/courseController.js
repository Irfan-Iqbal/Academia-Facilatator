...
exports.get_single_course = async (req, res) => {
  ...
  var user;
  if (
    (req.session.isLogged != null || req.session.isLogged != undefined) &&
  ...
  User.findById(req.session.user_id, (err, user) => {
    ...
    Course.find({ _id: { $in: purchasedCourseIds } }, async (err, courses) => {
      ...
      const size = await User.aggregate([
        { $match: { _id: req.session.user_id } },
        { $project: { courses: { $size: '$likedCourses' } } },
      ]);
...
exports.verify_payment = async (req, res) => {
  ...
  //     .update('khedjt338uy333' + '|' + req.body.razorpay_payment_id)
  //     .digest('hex');
  ...
  Course.findById(courseID, (err, course) => {
    ...
    instance.orders.create(options, function (err, order) {
      return res.render('checkout', {
        ...
        adminLogged: req.session.adminLogged,
        message:
          'Select wide range of online payment options using Razorpay.(UPI/ Net Banking/ Credit Card/ Debit Card/ External Wallets',
        course: course,
      });
    });
  }).select('-description -aboutInstructor');
